...
local function setup_keymaps()
    ...
    local function move_and_center(direction)
        return function()
            if vim.fn.mode() == "v" or vim.fn.mode() == "V" or vim.fn.mode() == "" then
                -- Visual mode: move selection and reselect
                vim.cmd("normal! " .. direction)
                vim.cmd("normal! zz")
                vim.cmd("normal! gv") -- reselect the visual region
            else
                -- Normal mode: move and center
                vim.cmd("normal! " .. direction)
                vim.cmd("normal! zz")
            end
        end
    ...
    vim.keymap.set({ "n", "v" }, "<C-j>", move_and_center("j"), { desc = "Move down and center" })
    vim.keymap.set({ "n", "v" }, "<C-k>", move_and_center("k"), { desc = "Move up and center" })
